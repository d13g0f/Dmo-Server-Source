...
namespace DigitalWorldOnline.Infrastructure.Repositories.Server
{
    public class ServerCommandsRepository : IServerCommandsRepository
    {
        ...
        public async Task UpdateServerAsync(long serverId, string serverName, int experience, bool maintenance, int experienceBurn, int experienceType)
        {
            var dto = await _context.ServerConfig
                .AsNoTracking()
            ...
            if (dto != null)
            {
                ...
                dto.ExperienceBurn = experienceBurn;
                dto.ExperienceType = experienceType;
