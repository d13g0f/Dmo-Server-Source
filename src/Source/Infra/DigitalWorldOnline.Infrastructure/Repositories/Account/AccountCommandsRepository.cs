...
namespace DigitalWorldOnline.Infrastructure.Repositories.Account
{
    public class AccountCommandsRepository : IAccountCommandsRepository
    {
        ...
        public async Task UpdateAccountDailyRewardAsync(long accountId, bool dailyRewardClaimed, int dailyRewardClaimedAmount)
        {
            var dto = await _context.Account.FirstOrDefaultAsync(x => x.Id == accountId);

            if (dto != null)
            {
                dto.DailyRewardClaimed = dailyRewardClaimed;
                dto.DailyRewardClaimedAmount = dailyRewardClaimedAmount;
