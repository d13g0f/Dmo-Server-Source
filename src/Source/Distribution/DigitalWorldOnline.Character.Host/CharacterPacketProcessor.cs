...
namespace DigitalWorldOnline.Character
{
    public sealed class CharacterPacketProcessor : IProcessor, IDisposable
    {
        ...
        public CharacterPacketProcessor(ILogger logger, ISender sender, IConfiguration configuration, IMapper mapper)
        {
                ...
                case CharacterServerPacketEnum.RequestCharacters:
                    {
                        ...
                        {
                            _logger.Error($"[ERROR - RequestCharacters]: {ex.Message}");
                ...
                case CharacterServerPacketEnum.CreateCharacter:
                    {
                        ...
                        {
                            _logger.Error($"[ERROR - CreateCharacter]: {ex.Message}");
                            _logger.Error($"{ex.InnerException}");
                        }
                    }
                    break;
                ...
                case CharacterServerPacketEnum.CheckNameDuplicity:
                    {
                        ...
                        {
                            _logger.Error($"[ERROR - CheckNameDuplicity]: {ex.Message}");
                            _logger.Error($"{ex.InnerException}");
                        }
                    }
                    break;
                ...
                case CharacterServerPacketEnum.DeleteCharacter:
                    {
                        ...
                        {
                            _logger.Error($"[ERROR - DeleteCharacter]: {ex.Message}");
                            _logger.Error($"{ex.InnerException}");
                        }
                    }
                    break;
                ...
                case CharacterServerPacketEnum.GetCharacterPosition:
                    {
                        ...
                        try
                        {
                            _logger.Information($"Getting Character Position...");
                            ...
                            client.Send(new ConnectGameServerInfoPacket(_configuration[GameServerAddress], _configuration[GameServerPort],
                                character.Location.MapId).Serialize());
                        }
                        catch (Exception ex)
                        {
                            _logger.Error($"[ERROR - GetCharacterPosition]: {ex.Message}");
                            _logger.Error($"{ex.InnerException}");
                        }
                    }
                    break;
                ...
                case CharacterServerPacketEnum.ConnectGameServer:
                    {
                        try
                        {
                            _logger.Information("Connecting to GameServer ...");
                            client.Send(new ConnectGameServerPacket().Serialize());
                        }
                        catch (Exception ex)
                        {
                            _logger.Error($"[ERROR - ConnectGameServer]: {ex.Message}");
                            _logger.Error($"{ex.InnerException}");
                        }
                    }
                    break;
