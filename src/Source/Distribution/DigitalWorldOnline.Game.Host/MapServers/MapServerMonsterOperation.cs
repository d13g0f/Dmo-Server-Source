...
namespace DigitalWorldOnline.GameHost
{
    public sealed partial class MapServer
    {
        private void MonsterOperation(GameMap map)
        {
            ...
            foreach (var mob in map.SummonMobs.ToList())
            {
                if (DateTime.Now > mob.ViewCheckTime)
                {
                    ...
                    //mob.TamersViewing.RemoveAll(x => !map.ConnectedTamers.Select(y => y.Id).Contains(x));
                    mob.TamersViewing.RemoveAll(tamerId => !map.ConnectedTamers.Any(client => client.Id == tamerId));
                    ...
                    if (!mob.Dead)
                    {
                        foreach (var nearTamerId in nearTamers)
                        {
                            if (!mob.TamersViewing.Contains(nearTamerId))
                            {
                                mob.TamersViewing.Add(nearTamerId);
                                var targetClient = map.Clients.FirstOrDefault(x => x.TamerId == nearTamerId);
                                targetClient?.Send(new LoadMobsPacket(mob, true));
                            }
                        }
                    }
                    ...
                    foreach (var farTamerId in farTamers)
                    {
                        if (mob.TamersViewing.Contains(farTamerId))
                        {
                            mob.TamersViewing.Remove(farTamerId);
                            var targetClient = map.Clients.FirstOrDefault(x => x.TamerId == farTamerId);
                            targetClient?.Send(new UnloadMobsPacket(mob));
                        }
                    }
                }
            ...
            if (totalTime >= 1000)
                _logger.Information($"[MapServerMonsterOperation] :: TotalMobs =  {map.Mobs.Count}, TotalSummon =  {map.SummonMobs.Count}, TimeToRun = {totalTime}.");
        }
        ...
        public long CalculateExperience(int tamerLevel, int mobLevel, long baseExperience)
        {
            ...
            // Verify if Tamer is 20 levels more than mob
        ...
        private async Task PartyExperienceReward(GameMap map, MobConfigModel mob, List<int> partyIdList, GameClient? targetClient,
            long tamerExpToReceive, ReceiveExpResult tamerResult, long partnerExpToReceive, ReceiveExpResult partnerResult)
        {
        ...
        private async Task PartyExperienceReward(GameMap map, SummonMobModel mob, List<int> partyIdList, GameClient? targetClient,
           long tamerExpToReceive, ReceiveExpResult tamerResult, long partnerExpToReceive, ReceiveExpResult partnerResult)
        {
            var party = _partyManager.FindParty(targetClient.TamerId);
            if (party != null && !partyIdList.Contains(party.Id))
        ...
        private async void BitDropReward(GameMap map, MobConfigModel mob, GameClient? targetClient)
        {
            ...
            // System for multiply bits by Acceslevel
            ...
            if (bitsReward.Chance >= UtilitiesFunctions.RandomDouble())
            {
                ...
                await _sender.Send(new UpdateItemListBitsCommand(targetClient.Tamer.Inventory.Id, targetClient.Tamer.Inventory.Bits));
            }
        }

        private void ItemDropReward(GameMap map, MobConfigModel mob, GameClient? targetClient)
        {
            if (targetClient == null || !mob.DropReward.Drops.Any())
                return;

            Dictionary<int, ItemAssetModel> _itemInfoCache = _assets.ItemInfo.ToDictionary(x => x.ItemId, x => x);

            var itemsReward = new List<ItemDropConfigModel>();

            itemsReward.AddRange(mob.DropReward.Drops.Where(x => !_assets.QuestItemList.Contains(x.ItemId)));
            ...
            var dropGlobal = _configs.GlobalDrops.Where(x => x.Map == map.MapId).ToList();

            foreach (var itemDrop in dropGlobal)
            {
                if (DateTime.Now >= itemDrop.StartTime && DateTime.Now <= itemDrop.EndTime)
                {
                    if (itemDrop.Map == 0 || itemDrop.Map == targetClient.Tamer.Location.MapId)
                    {
                        var newItemDrops = new List<ItemDropConfigModel>();

                        var newItem = new ItemDropConfigModel
                        {
                            ItemId = itemDrop.ItemId,
                            MinAmount = itemDrop.MinDrop * vipMultiplier,
                            MaxAmount = itemDrop.MaxDrop * vipMultiplier,
                            Chance = itemDrop.Chance
                        };

                        newItemDrops.Add(newItem);
                        itemsReward.AddRange(newItemDrops);
                    }
                }
            }
            ...
            var floorDrops = new List<Drop>();

            Random random = new Random();
            
            foreach (var itemDrop in itemsReward)
            {
                if (random.Next(0, 100) <= itemDrop.Chance)
                {
                    var newItem = new ItemModel { ItemId = itemDrop.ItemId };
                    newItem.SetItemInfo(_itemInfoCache.GetValueOrDefault(itemDrop.ItemId));
                    ...
                    if (targetClient.Tamer.HasAura && targetClient.Tamer.Aura.ItemInfo.Section == 2100)
                    {
                        ...
                        {
                            floorDrops.Add(CreateMapDrop(map, mob, targetClient, itemDrop));
                        }
                    }
                    else
                    {
            ...
            // Enviar pacotes de itens recebidos e atualizar o inventário uma vez
            foreach (var item in droppedItems)
            {
                targetClient.Send(new ReceiveItemPacket(item, InventoryTypeEnum.Inventory));
            }
            ...
            // Adicionar drops no chão ao mapa (em lote, se necessário)
        ...
        private async void RaidReward(GameMap map, MobConfigModel mob)
        {
            ...
            foreach (var raidTamer in keyValuePairs.OrderByDescending(x => x.Value))
            {
                ...
                {
                    int maxRank = raidRewards.Max(x => x.Rank);

                    var rewards = raidRewards.Where(x => x.Rank == i);
                    var itemDropConfigModels = rewards.ToList();

                    if (itemDropConfigModels.Count == 0)
                    {
                        rewards = raidRewards.Where(x => x.Rank == maxRank);
                        itemDropConfigModels = rewards.ToList();
                    }
            ...
            await BlessingFIW(mob, attackerName, attackerType);

            map.BroadcastForTargetTamers(mob.RaidDamage.Select(x => x.Key).ToList(), writer.Serialize());
            updateItemList.ForEach(itemList => { _sender.Send(new UpdateItemsCommand(itemList)); });

            await OmegamonBlessing(mob, attackerName, attackerType);
        }
            ...
            if (mob.Bless)
            {
                for (int x = 0; x < 8; x++)
                {
                    ...
                    if (currentMap != null)
                    {
                        ...
                        var targetItem = _assets.ItemInfo.FirstOrDefault(itemAssetModel => itemAssetModel.ItemId == 71552);
        ...
        private async Task OmegamonBlessing(MobConfigModel mob, string attackerName, int attackerType)
        {
            if (mob.Buff)
            {
                for (int x = 0; x < 8; x++)
                {
                    ...
                    if (currentMap != null)
                    {
                        ...
                        var targetItem =
                            _assets.ItemInfo.FirstOrDefault(itemAssetModel => itemAssetModel.ItemId == 992272);
        ...
        private void BitDropReward(GameMap map, SummonMobModel mob, GameClient targetClient)
        {
            ...
            if (bitsReward.Chance >= UtilitiesFunctions.RandomDouble())
            {
                ...
                _logger.Information($"Tamer [{targetClient.TamerId}:{targetClient.Tamer.Name}] get {amount} bits");
            }
        }
        ...
        private void ItemDropReward(GameMap map, SummonMobModel mob, GameClient? targetClient)
        {
            ...
            var itemsReward = mob.DropReward.Drops.Where(x => !_assets.QuestItemList.Contains(x.ItemId)).ToList();
            ...
            var shuffledDrops = itemsReward.OrderBy(x => Guid.NewGuid()).ToList();

            while (dropped < totalDrops)
            {
                if (!shuffledDrops.Any())
                {
                ...
                var itemDrop = shuffledDrops.First();
                shuffledDrops.RemoveAt(0);
                ...
                if (itemDrop.Chance >= UtilitiesFunctions.RandomDouble())
                {
                    ...
                    if (newItem.ItemInfo == null)
                    {
                        _logger.Warning($"No item info found with ID {itemDrop.ItemId} for tamer {targetClient?.TamerId}.");
                        continue;
                    }
                    ...
                    if (targetClient?.Tamer != null && targetClient.Tamer.HasAura && targetClient.Tamer.Aura.ItemInfo?.Section == 2100)
                    {
                        var itemClone = (ItemModel)newItem.Clone();
                        ...
                        {
                            targetClient.Send(new ReceiveItemPacket(itemClone, InventoryTypeEnum.Inventory));
                            _sender.Send(new UpdateItemCommand(itemClone));
                            _logger.Verbose($"Character {targetClient.TamerId} aquired {newItem.ItemId} x{newItem.Amount} from mob {mob.Id} with magnetic aura {targetClient.Tamer.Aura.ItemId}.");
                        }
                        else
                        {
                            targetClient.Send(new PickItemFailPacket(PickItemFailReasonEnum.InventoryFull));
                            CreateAndDropItemOnMap(map, mob, targetClient, itemDrop);
                        }
                    }
                    else
                    {
        ...
        private Drop CreateMapDrop(GameMap map, MobConfigModel mob, GameClient? targetClient, ItemDropConfigModel itemDrop)
        {
        ...
        private void CreateAndDropItemOnMap(GameMap map, SummonMobModel mob, GameClient? targetClient, SummonMobItemDropModel itemDrop)
        {
            var drop = _dropManager.CreateItemDrop(
                targetClient!.Tamer.Id,
                targetClient!.Tamer.GeneralHandler,
