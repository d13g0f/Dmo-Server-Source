...
namespace DigitalWorldOnline.GameHost
{
    public sealed partial class DungeonsServer
    {
        private void MonsterOperation(GameMap map)
        {
            ...
            if (totalTime >= 1000)
                _logger.Information($"[DungeonServerMonsterOperation] :: Mobs Count =  ({map.Mobs.Count}), TimeToRun = {totalTime}.");
        }
        ...
        private void RaidReward(GameMap map, MobConfigModel mob)
        {
            ...
            {
                _logger.Debug($"Character {raidTamer.Key} rank {i} on raid {mob.Id} - {mob.Name} with damage {raidTamer.Value}.");
