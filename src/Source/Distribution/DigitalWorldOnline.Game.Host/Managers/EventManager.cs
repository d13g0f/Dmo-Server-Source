        ...
        public async Task EventServer(MapServer map)
        {
            var mobsList = new Dictionary<int, SummonModel?>
                {
                    ...
                    { 4, _assets.SummonInfo.FirstOrDefault(x => x.Id == 68) },
                  //  { 5, _assets.SummonInfo.FirstOrDefault(x => x.Id == 75) }
                };

            if (roundCooldownStartTime != DateTime.MinValue && (DateTime.Now - roundCooldownStartTime).TotalSeconds < roundCooldownDuration)
                return;
            ...
            if ((DateTime.Now - roundStartTime).TotalSeconds >= roundDurationInSeconds)
            {
                if (currentRound < mobsList.Count)
                {
                    ...
                    {
                        map.BroadcastGlobal(new NoticeMessagePacket($"Round {currentRound} has ended. Next round will start in {roundCooldownDuration} seconds...").Serialize());
