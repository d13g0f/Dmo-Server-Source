    ...
    public class PartnerDigiclonePacketProcessor : IGamePacketProcessor
    {
        ...
        public async Task Process(GameClient client, byte[] packetData)
        {
                ...
                !UtilitiesFunctions.IsCloneItem(digicloneItem.ItemInfo.Section))
            {
                ...
                client.SendToAll(new NoticeMessagePacket($"Tamer: {client.Tamer.Name} tried to clone their digimon using a cheat method, Then they got banned!").Serialize());

                //var banProcessor = SingletonResolver.GetService<BanForCheating>();
                //banProcessor.BanAccountWithMessage(client.AccountId, client.Tamer.Name, AccountBlockEnum.Permanent, "Cheating", client, "You tried to clone your digimon using a cheat method, So be happy with ban!");
