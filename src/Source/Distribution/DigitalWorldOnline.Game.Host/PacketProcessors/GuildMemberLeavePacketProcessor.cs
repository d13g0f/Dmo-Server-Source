...
namespace DigitalWorldOnline.Game.PacketProcessors
{
    public class GuildMemberLeavePacketProcessor : IGamePacketProcessor
    {
        ...
        {
            var targetGuild = client.Tamer.Guild;

            _logger.Debug($"Searching guild by character id {client.TamerId} - {client.Tamer.Name} ...");
            
            if (targetGuild == null)
            {
                _logger.Warning($"Character {client.TamerId} - {client.Tamer.Name} does not belong to a guild.");
                client.Send(new SystemMessagePacket($"Character {client.TamerId} - {client.Tamer.Name} does not belong to a guild."));
                return;
            }
