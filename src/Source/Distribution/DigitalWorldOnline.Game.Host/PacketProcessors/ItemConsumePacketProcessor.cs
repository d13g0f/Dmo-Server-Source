...
namespace DigitalWorldOnline.Game.PacketProcessors
{
    public class ItemConsumePacketProcessor : IGamePacketProcessor
    {
        ...
        public ItemConsumePacketProcessor(
            ...
            AssetsLoader assets, ExpManager expManager, ConfigsLoader configs,
        ...
        {
            // Criar uma nova instância do Stopwatch e inicia
            ...
            {
                await ConsumeResetSkill(client, itemSlot, targetItem);
            }
            else if (targetItem.ItemInfo.Type == 63)
            {
                await BuffItem(client, itemSlot, targetItem);
            }
            else if (targetItem.ItemInfo.Type == 67 || targetItem.ItemInfo.Type == 68 || targetItem.ItemInfo.Type == 69)
            {
                await ConsumeMemorySkills(client, itemSlot, targetItem);
            }
            ...
            {
                _logger.Error($"Item [{targetItem.ItemId} : {targetItem.ItemInfo.Name}] not configured !!");
                client.Send(new ItemConsumeFailPacket(itemSlot, targetItem.ItemInfo.Type));
            }

            // Finaliza o contador e mostra no console
            //stopwatch.Stop();
            //TimeSpan tempoDecorrido = stopwatch.Elapsed;
            //_logger.Information($"Tempo decorrido: {tempoDecorrido}");
        }
        ...
        private async Task ConsumeMemorySkills(GameClient client, short itemSlot, ItemModel targetItem)
        {
            ...
            {
                var targetSkill = _assets.DigimonSkillInfo.FirstOrDefault(x => x.Type == client.Partner.CurrentType);
                
                if (targetSkill != null)
                {
                    //targetItem.ItemInfo?.SetSkillInfo(_assets.SkillCodeInfo.FirstOrDefault(x => x.SkillCode == targetSkill.SkillId));
                }
        ...
        private async Task SummonMonster(GameClient client, short itemSlot, ItemModel targetItem, SummonModel? SummonInfo)
        {
            if (SummonInfo == null)
            {
                _logger.Error($"SummonInfo not found !! [ItemConsume]");
            }
            else
            {
                //_logger.Information($"AllMaps: {SummonInfo.allMaps}");

                if (!SummonInfo.Maps.Contains(client.Tamer.Location.MapId) && SummonInfo.allMaps == 0)
                {
                    _logger.Debug($"Summon map not available !! [ItemConsume]");

                    client.Send(new ItemConsumeFailPacket(itemSlot, targetItem.ItemInfo!.Type, ItemConsumeFailEnum.InvalidArea));
                }
                else
                {
                    //_logger.Information($"Summon item liberado ...");
                    ...
                    foreach (var mobToAdd in SummonInfo.SummonedMobs)
                    {
                        ...
                        if (mob?.Location?.X != 0 && mob?.Location?.Y != 0 && SummonInfo.allMaps == 0)
                        {
                            ...
                            {
                                //_logger.Information($"diff location > 5000: {diff}");
                        ...
                        //_logger.Information($"Dispersando mob {count} - {mob.Name} !!");
                        ...
                        if (client.DungeonMap)
                        {
                            ...
                            //_logger.Information($"MapInfo: {map.MapId} - {map.Name}");
                            ...
                            if (mob?.Location?.X != 0 && mob?.Location?.Y != 0 && SummonInfo.allMaps == 0)
                            {
                                ...
                                //_logger.Information($"Mob Location: {bossX}, {bossY}");
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                if (mob?.Location?.X != 0 && mob?.Location?.Y != 0 && SummonInfo.allMaps == 0)
                                {
                                    ...
                                    //_logger.Information($"New Mob Location: {bossX}, {bossY}");
                    ...
                    //_logger.Information($"Todos os mobs foram spawnados !!");
                }
            }
        }
        ...
        private async Task ConsumeResetSkill(GameClient client, short itemSlot, ItemModel targetItem)
        {
            client.Tamer.Inventory.RemoveOrReduceItem(targetItem, 1);
        ...
        private async Task BombTeleport(GameClient client, short itemSlot, ItemModel targetIte)
        {
            ...
            else
            {
                ...
                _logger.Error($"ItemID {targetIte.ItemId} não encontrado no mapeamento.");
            }
        }
        ...
        private async Task ContainerItem(GameClient client, short itemSlot, ItemModel targetItem)
        {
            ...
            else
            {
                ...
                if (ItemId == 70102)
                {
                    ...
                    int time = 30 * 24 * 3600;  // (30 * 24 * 3600) 30 Days Membership | (60 * 24 * 3600) 60 Days Membership

                    client.IncreaseMembershipDuration(time);

                    client.Send(new MembershipPacket(client.MembershipExpirationDate!.Value, client.MembershipUtcSeconds));

                    var buffData = _assets.BuffInfo.Where(x => x.BuffId == 50121 || x.BuffId == 50122 || x.BuffId == 50123).ToList();

                    int duration = client.MembershipUtcSecondsBuff;

                    buffData.ForEach(buffAsset =>
                    {
                        if (!client.Tamer.BuffList.Buffs.Any(x => x.BuffId == buffAsset.BuffId))
                        {
                            ...
                            {
                                case MapTypeEnum.Dungeon:
                                    _dungeonServer.BroadcastForTamerViewsAndSelf(client.TamerId, new AddBuffPacket(client.Tamer.GeneralHandler, buffAsset, 0, duration).Serialize());
                                    break;

                                case MapTypeEnum.Event:
                                    _eventServer.BroadcastForTamerViewsAndSelf(client.TamerId, new AddBuffPacket(client.Tamer.GeneralHandler, buffAsset, 0, duration).Serialize());
                                    break;

                                case MapTypeEnum.Pvp:
                                    _pvpServer.BroadcastForTamerViewsAndSelf(client.TamerId, new AddBuffPacket(client.Tamer.GeneralHandler, buffAsset, 0, duration).Serialize());
                                    break;
                        ...
                        else
                        {
                            ...
                            if (buffData != null)
                            {
                                ...
                                {
                                    case MapTypeEnum.Dungeon:
                                        _dungeonServer.BroadcastForTamerViewsAndSelf(client.TamerId, new UpdateBuffPacket(client.Tamer.GeneralHandler, buffAsset, 0, duration).Serialize());
                                        break;

                                    case MapTypeEnum.Event:
                                        _eventServer.BroadcastForTamerViewsAndSelf(client.TamerId, new UpdateBuffPacket(client.Tamer.GeneralHandler, buffAsset, 0, duration).Serialize());
                                        break;

                                    case MapTypeEnum.Pvp:
                                        _pvpServer.BroadcastForTamerViewsAndSelf(client.TamerId, new UpdateBuffPacket(client.Tamer.GeneralHandler, buffAsset, 0, duration).Serialize());
                                        break;
