    ...
    public class QuestAcceptPacketProcessor : IGamePacketProcessor
    {
        ...
        public async Task Process(GameClient client, byte[] packetData)
        {
            ...
            try
            {
                ...
                if (questInfo == null)
                {
                    _logger.Error($"[QuestAccept] :: Tamer {client.Tamer.Name} tryed to accept Quest with Id {questId} not in database !!");
                    client.Disconnect();
                    return;
                }
                ...
                if (client.Tamer.Progress.AcceptQuest(questId))
                {
                    ...
                    //_logger.Information($"Tamer {client.TamerId}:{client.Tamer.Name} accepted quest {questId}.");
            ...
            {
                _logger.Error($"[QuestAccept] :: {ex.Message}");
            }
        }
    }
}
