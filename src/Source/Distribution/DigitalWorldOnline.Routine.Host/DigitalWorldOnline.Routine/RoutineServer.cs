...
namespace DigitalWorldOnline.Routine
{
    public sealed class RoutineServer : IHostedService
    {
        ...
        public RoutineServer(IHostApplicationLifetime hostApplicationLifetime, AssetsLoader assets, ILogger logger, ISender sender, IMapper mapper)
        {
        ...
        public async Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                foreach (var routine in routines.Where(x => x.ExecutionTime))
                {
                    ...
                    _logger.Information($"Updating {routine.Name} execution time ... [30 seconds]");

                    await Task.Delay(30000, cancellationToken);
                    await _sender.Send(new UpdateRoutineExecutionTimeCommand(routine.Type), cancellationToken);
                }

                _logger.Information($"Routines updated !! Next routines execution in 5 minutes ...");
