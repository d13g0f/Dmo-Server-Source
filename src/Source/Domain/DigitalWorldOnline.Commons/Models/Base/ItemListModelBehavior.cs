...
namespace DigitalWorldOnline.Commons.Models.Base
{
    public partial class ItemListModel
    {
        ...
        public void Sort()
        {
            ...
            var emptyItens = Items.Where(x => x.ItemId == 0).ToList();
        ...
        public void CheckEmptyItems()
        {
            Items.ForEach(item =>
            {
                if (item.ItemId == 0 || item.Amount <= 0)
                {
                    ...
                    item.SetRemainingTime();
                    item.SetSellPrice(0);
                    item.SetItemInfo(null); // Verificar se deu bo nos itens !! se sim comentar
                }
            });
        }
        ...
        public byte[] ToArray()
        {
            ...
            using (MemoryStream m = new())
            {
                ...
                {
                    byte[] itemBuffer = item.ToArray();
                    //Console.WriteLine($"Item (Slot: {item.Slot}): Tamanho do array = {itemBuffer.Length}");
                    m.Write(itemBuffer, 0, 68);
                }
